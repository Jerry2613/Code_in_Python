...
class SkipAction(object):
    ...
    def check_condition_para(used_token_dict, line):
        ...
        if len(s_line_list) == 1:
            if re.search('!', s_line, re.IGNORECASE):
                ...
                data = used_token_dict.get(new_line, 'None')
                condition_value = True if data in zero_none_list else False
            else:
                data = used_token_dict.get(s_line, 'None')
                condition_value = True if data not in zero_none_list else False

        if len(s_line_list) == 2:
            data = used_token_dict.get(s_line_list[1], 'None')
            if re.match('!defined ', s_line, re.IGNORECASE):
                condition_value = True if data == 'None' else False
            if re.match('defined ', s_line, re.IGNORECASE):
                condition_value = True if data != 'None' else False
        ...
        if len(s_line_list) == 3:
            for ll in logical_list:
                if re.search(ll, s_line, re.IGNORECASE):
                    ...
                    data = used_token_dict.get(s_line_n3_list[0].strip(), 'None')
                    right_data = used_token_dict.get(s_line_n3_list[1].strip(), 'None')
                    if ll == '==':
    ...
    def check_condition_para_suppressif(line, efi_variable, token_dict):
        ...
        if re.search('ideqval ', s_line, re.IGNORECASE):
            ...
            # print('=====', new_line_2_list, 'l_value', l_value, 'r_value', r_value)
            if l_value == r_value:
                condition_value = False if not_flag else True
                # print('=====1 condition_value=', condition_value, '***not_flag:', not_flag)
            else:
                condition_value = True if not_flag else False
                # print('=====2 condition_value=', condition_value, '***not_flag:', not_flag)
    ...
    def check_line_suppressif_status(line, efi_variable, token_dict):
        ...
        if re.match('suppressif ', s_line, re.IGNORECASE):
            ...
            total_or_num = new_line.count(' OR ')
            total_and_num = new_line.count(' AND ')
            or_location = 100
            and_location = 100
            if total_or_num + total_and_num == 0:
                para_list.append(new_line)
            for i in range(total_or_num + total_and_num):
                if total_or_num >= 1:
                    or_location = new_line.index(' OR ')
                if total_and_num >= 1:
                    and_location = new_line.index(' AND ')
                if or_location < and_location:
                    new_line_list = new_line.split(' OR ', 1)
                    total_or_num -= 1
                    link_list.append('OR')
                else:
                    new_line_list = new_line.split(' AND ', 1)
                    total_and_num -= 1
                    link_list.append('AND')
                para_list.append(new_line_list[0].strip())
                new_line = new_line_list[1].strip()
