...
class FileDealWith(object):
    ...
    def get_priority_number(self, filepath):
        # priority [1] > [0]
        override_priority_list = ['OEMBOARD', 'Livingstone2']
        override_priority = 0
        if re.search(override_priority_list[0], filepath, re.IGNORECASE):
            override_priority = 16
            if re.search(override_priority_list[1], filepath, re.IGNORECASE):
