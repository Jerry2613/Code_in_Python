...
class RuggedDigest():
    ...
    def produce_digest_table(self, service_raw_data, mac_raw_data, digestfilename):
        ...
        for s, m in zip(service_raw_data, mac_raw_data):
            ...
            hex_digest.append('0x' + self.hexreverse(hash_object.hexdigest()[0:8]) + ', ')
            hex_digest.append('0x' + self.hexreverse(hash_object.hexdigest()[8:16]) + ', ')
            hex_digest.append('0x' + self.hexreverse(hash_object.hexdigest()[16:24]) + ', ')
            hex_digest.append('0x' + self.hexreverse(hash_object.hexdigest()[24:32]) + ', ')
            hex_digest.append('0x' + self.hexreverse(hash_object.hexdigest()[32:40]) + ', ')
 ...
 #       pdb.set_trace()
        if len(sys.argv) == 2 or len(sys.argv) == 3:
            if not (os.path.isfile(sys.argv[1])):
                # FileNotFoundError is a subclass of OSError, which takes several arguments.
                # The first is an error code from the errno module (file not found is always errno.ENOENT),
                # the second the error message (use os.strerror() to obtain this),
                # and pass in the filename as the 3rd.
