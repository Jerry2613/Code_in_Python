...
class RuggedDigest():
    ...
    def produce_digest_table(self, service_raw_data, mac_raw_data, digestfilename):
        for s, m in zip(service_raw_data, mac_raw_data):
            ...
            self.hex_digest.append('0x' + RuggedDigest.hexreverse(hash_object.hexdigest()[0:8]))
            self.hex_digest.append('0x' + RuggedDigest.hexreverse(hash_object.hexdigest()[8:16]))
            self.hex_digest.append('0x' + RuggedDigest.hexreverse(hash_object.hexdigest()[16:24]))
            self.hex_digest.append('0x' + RuggedDigest.hexreverse(hash_object.hexdigest()[24:32]))
            self.hex_digest.append('0x' + RuggedDigest.hexreverse(hash_object.hexdigest()[32:40]))
        with open(digestfilename, mode='w') as Hash_table:
            i = 0
            for h in self.hex_digest:
                i += 1
                Hash_table.write(h + ', ')
                if i % 5 == 0:
                    Hash_table.write("\n")
...
if __name__ == '__main__':
    try:
        ...
        if os.path.isfile(sys.argv[1]):
            ...
            e.produce_digest_table(e.s_raw_data, e.m_raw_data, e.digestfilename)
            e.produce_table_file(e.lv_s_raw_data, e.lv_servicetagfilename)
            e.produce_table_file(e.lv_m_raw_data, e.lv_macfilename)
            e.produce_digest_table(e.lv_s_raw_data, e.lv_m_raw_data, e.lv_digestfilename)
            e.showtable()
